...
func (b *BBS) Index(c echo.Context) error {
	...
	err := b.DB.Order("id desc").Find(&cos).Error
	if err != nil {
...
func (b *BBS) Show(c echo.Context) error {
	...
	err := b.DB.Where("id = ?", id).Find(&comment).Error
	if err != nil {
		return c.Render(http.StatusOK, "err", err.Error())
	}
	data := struct {
...
func (b *BBS) Update(c echo.Context) error {
	...
	err := b.DB.Model(&co).Where("id = ?", id).Update(model.Comments{Name: name, Comment: comment}).Error
	if err != nil {
		return c.Render(http.StatusOK, "err", err.Error())
	}
	return c.Redirect(http.StatusMovedPermanently, "/")
...
func (b *BBS) Delete(c echo.Context) error {
	id := c.FormValue("id")
	var co model.Comments
	err := b.DB.Where("id = ?", id).Delete(&co).Error
	if err != nil {
		return c.Render(http.StatusOK, "err", err.Error())
	}
	return c.Redirect(http.StatusMovedPermanently, "/")
